...
function generateRandomKeywords($count = 5) {
    global $keywordsPool;
    shuffle($keywordsPool);
    return implode(',', array_slice($keywordsPool, 0, $count));
}
...
function getCategoryStats() {
    ...
    foreach ($categories as $category) {
        $stats[$category] = $categoryMaxCounts[$category] ?? 0;
    }
...
function filterArticlesBySearch($articles, $searchTerm) {
    ...
    foreach ($articles as $article) {
        if (strpos(strtolower($article['title']), $searchTerm) !== false) {
            $filtered[] = $article;
        }
    }
...
function generateUrl($params = []) {
    $defaultParams = [
        ...
        'p' => $GLOBALS['currentPage'],
        'q' => $GLOBALS['searchTerm']
    ];
...
function getCategoryArticles($category, $page = 1) {
    ...
    $maxId = min(20, $categoryMaxCounts[$category] ?? 20);
...
function getAllArticles($page = 1) {
    ...
    $allArticles = [];
    foreach ($categories as $category) {
        $maxId = min(10, $categoryMaxCounts[$category] ?? 10);
        $articles = loadArticlesFromCategory($category, 1, $maxId);
    ...
    <div class="container mx-auto p-4 md:p-8 max-w-6xl">
                ...
                <?php foreach ($categories as $category): ?>
                    <a href="<?php echo generateUrl(['category' => $category, 'p' => 1]); ?>"
                       class="nav-link text-blue-500 hover:text-blue-700 whitespace-nowrap <?php echo $currentCategory === $category ? 'active' : ''; ?>">
                        <?php echo htmlspecialchars($categoryNames[$category]); ?>
                    </a>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div id="category-stats" class="mb-6 p-4 bg-white rounded-lg shadow-sm">
            ...
            foreach ($categoryStats as $category => $count):
                echo "<div><span class='text-gray-700'>{$categoryNames[$category]}:</span> <span class='font-medium'>{$count}</span> 篇</div>";
            endforeach;
            ...
            <div id="article-detail">
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm mb-6">
                    ...
                    <div class="text-gray-500 text-sm mb-6">
                        <span class="mr-4"><i class="fas fa-folder text-gray-400 mr-1"></i> <?php echo htmlspecialchars($categoryNames[$articleDetail['category']]); ?></span>
                        <span><i class="fas fa-file-alt text-gray-400 mr-1"></i> ID: <?php echo htmlspecialchars($articleDetail['id']); ?></span>
                        <?php if ($articleDetail['isMarkdown']): ?>
                ...
                <div class="flex justify-between">
                    ...
                    $hasPrev = $prevId >= 1;
                    $hasNext = $nextId <= ($categoryMaxCounts[$currentCategory] ?? 1000);
                    ?>
        ...
        <?php else: ?>
            ...
            <div id="article-list-container" class="mb-8">
                <div id="article-list" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php if (!empty($articleListData['articles'])): ?>
                                ...
                                <a href="<?php echo generateUrl(['id' => $article['id'], 'category' => $article['category']]); ?>" class="block h-full">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <span class="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded-full">
                                            <?php echo htmlspecialchars($categoryNames[$article['category']]); ?>
                                        </span>
                                    </div>
                ...
                <!-- 分页控件 -->
                <?php if ($articleListData['totalPages'] > 1): ?>
