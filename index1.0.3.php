...
// 配置参数
$categories = array('category1', 'category2', 'category3', 'category4', 'category5', 'category6', 'category7');
$categoryNames = array(
    'category1' => '诗词歌赋',
    'category2' => '历史典故',
    'category3' => '科技前沿',
...
function loadArticle($category, $id) {
    ...
    $lines = explode("\n", $fileContent);
    $title = trim(isset($lines[0]) ? $lines[0] : "未命名文章");
    $description = trim(isset($lines[1]) ? $lines[1] : "");
...
function countArticlesInCategory($category) {
    if (empty($category) || !is_dir($category)) {
        return 0;
    }
...
function getCategoryStats() {
    ...
    foreach ($categories as $category) {
        $stats[$category] = countArticlesInCategory($category);
    }
...
function getCategoryArticles($category, $page = 1) {
    if (empty($category) || !is_numeric($page)) {
        return array('articles' => array(), 'total' => 0, 'totalPages' => 1);
    }
    
    global $itemsPerPage;
    
    // 获取实际文章数量
    $totalArticles = countArticlesInCategory($category);
    $maxId = min(20, $totalArticles); // 限制单次加载数量，避免性能问题
...
function getAllArticles($page = 1) {
    ...
    global $itemsPerPage, $categories;
    
    $allArticles = array();
    foreach ($categories as $category) {
        $totalArticles = countArticlesInCategory($category);
        $maxId = min(10, $totalArticles); // 限制每个分类加载数量
        $articles = loadArticlesFromCategory($category, 1, $maxId);
...
if ($articleId && $currentCategory) {
    ...
    if ($articleDetail) {
        ...
        $pageDescription = $articleDetail['description'] ? $articleDetail['description'] : $pageDescription;
        $pageKeywords = generateRandomKeywords();
    }
}

// 获取文章列表数据
$articleListData = array('articles' => array(), 'total' => 0, 'totalPages' => 1);
if (!$articleDetail) {
    if ($currentCategory) {
    ...
    <div class="container mx-auto p-4 md:p-8 max-w-6xl">
        ...
        <div id="category-nav" class="mb-6 overflow-x-auto pb-2">
            <div class="flex space-x-4 min-w-max">
                <a href="<?php echo generateUrl(array('category' => null, 'p' => 1)); ?>" 
                   class="nav-link text-blue-500 hover:text-blue-700 whitespace-nowrap <?php echo !$currentCategory ? 'active' : ''; ?>">
                    全部文章
                </a>
                
                <?php foreach ($categories as $category): ?>
                    <a href="<?php echo generateUrl(array('category' => $category, 'p' => 1)); ?>"
                       class="nav-link text-blue-500 hover:text-blue-700 whitespace-nowrap <?php echo $currentCategory === $category ? 'active' : ''; ?>">
                        <?php echo htmlspecialchars(isset($categoryNames[$category]) ? $categoryNames[$category] : $category); ?>
                        <?php $count = isset($categoryStats[$category]) ? $categoryStats[$category] : 0; ?>
                        <span class="ml-1 text-xs bg-gray-200 px-1 rounded">(<?php echo $count; ?>)</span>
                    </a>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div id="category-stats" class="mb-6 p-4 bg-white rounded-lg shadow-sm">
            ...
            foreach ($categoryStats as $category => $count):
                echo "<div><span class='text-gray-700'>".(isset($categoryNames[$category]) ? $categoryNames[$category] : $category).":</span> <span class='font-medium'>{$count}</span> 篇</div>";
            endforeach;
            ...
            <div id="article-detail">
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm mb-6">
                    ...
                    <div class="text-gray-500 text-sm mb-6">
                        <span class="mr-4"><i class="fas fa-folder text-gray-400 mr-1"></i> <?php echo htmlspecialchars(isset($categoryNames[$articleDetail['category']]) ? $categoryNames[$articleDetail['category']] : $articleDetail['category']); ?></span>
                ...
                <div class="flex justify-between">
                    ...
                    $totalArticles = countArticlesInCategory($currentCategory);
                    $hasPrev = $prevId >= 1;
                    $hasNext = $nextId <= $totalArticles;
                    ?>
                    
                    <a href="<?php echo $hasPrev ? generateUrl(array('id' => $prevId)) : '#'; ?>" 
                    ...
                    <a href="<?php echo $hasNext ? generateUrl(array('id' => $nextId)) : '#'; ?>" 
            ...
            <div id="article-list-container" class="mb-8">
                    ...
                    <?php if (!empty($articleListData['articles']) && is_array($articleListData['articles'])): ?>
                        <?php foreach ($articleListData['articles'] as $article): ?>
                            <div class="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition">
                                <a href="<?php echo generateUrl(array('id' => $article['id'], 'category' => $article['category'])); ?>" class="block h-full">
                                    <div class="flex justify-between items-start">
                                        <h3 class="font-medium text-gray-800 hover:text-blue-600 line-clamp-2">
                                            <?php echo htmlspecialchars(isset($article['title']) ? $article['title'] : '无标题'); ?>
                                        </h3>
                                        <span class="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded-full">
                                            <?php echo htmlspecialchars(isset($categoryNames[$article['category']]) ? $categoryNames[$article['category']] : $article['category']); ?>
                                        </span>
                                    </div>
                                    <p class="text-gray-500 text-sm mt-2">ID: <?php echo htmlspecialchars(isset($article['id']) ? $article['id'] : ''); ?></p>
                                </a>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                ...
                <?php if (!empty($articleListData['totalPages']) && $articleListData['totalPages'] > 1): ?>
                    <div id="pagination" class="flex flex-wrap justify-center gap-2 mt-6">
                        <a href="<?php echo generateUrl(array('p' => 1)); ?>" 
                        ...
                        <a href="<?php echo generateUrl(array('p' => $currentPage - 1)); ?>" 
                           class="bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600 transition <?php echo $currentPage == 1 ? 'bg-blue-700 pointer-events-none' : ''; ?>">
                        ...
                        ?>
                            <a href="<?php echo generateUrl(array('p' => $i)); ?>" 
                        ...
                        <a href="<?php echo generateUrl(array('p' => $articleListData['totalPages'])); ?>" 
                           class="bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600 transition <?php echo $currentPage == $articleListData['totalPages'] ? 'bg-blue-700 pointer-events-none' : ''; ?>">
                            末页
                        </a>
                    </div>
